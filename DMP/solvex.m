...
%yx(p.nz + 1 : 2*p.nz,        1 :   p.nz) = -p.B^(1/p.alpha)*((1-p.alpha)/p.alpha)*((1-p.gamma)/p.kappa)^((1-p.alpha)/p.alpha)*(p.beta*p.Pi*Si).^(1/p.alpha-2)*p.beta*p.Pi; % this is  Eq (18)
yx(p.nz + 1 : 2*p.nz,        1 :   p.nz) = -p.B^(1/p.alpha)*((1-p.gamma)/p.kappa)^((1-p.alpha)/p.alpha)*((1-p.alpha)/p.alpha)*(p.beta*p.Pi).^((1-p.alpha)/p.alpha)
 
; % derivative of second equation wrt to Si
yx(p.nz + 1 : 2*p.nz, p.nz + 1 : 2*p.nz) = eye(p.nz);
